<template>
  <!-- Gemeinsamer Anlege-Dialog für beide Wege: den Feedback-Button in der
       Kopfzeile (mit Screenshot) und den „Neues Ticket"-Button auf der
  ...
  <q-dialog id="ticket-anlegen-dialog" v-model="dialogOpen"
    ...
    <q-card class="vtb-feedback-dialog" :class="{ 'vtb-dialog-mobil': $q.screen.lt.sm }"
      ...
      <q-card-section class="row items-center q-pb-none">
        ...
        <div class="text-subtitle1 text-weight-bold q-ml-sm">Neues Ticket</div>
        <q-space />
      ...
      <q-card-section class="q-gutter-sm vtb-dialog-body">
        ...
        <!-- Sonst: Fotos (Rückkamera) und/oder hochgeladene Bilder — z. B. um vor
             Ort etwas zu belegen. Mehrere Bilder je Ticket möglich. -->
...
async function captureFullPage() {
  ...
  const canvas = await h2c(doc, {
    ...
    // Der Dialog selbst (samt Backdrop) und der Auslöse-Button gehören nicht ins Bild.
    ignoreElements: (el) =>
      el.id === 'feedback-fab' ||
...
// ── Öffnen: Formular zurücksetzen, ggf. Screenshot aufnehmen ─────────
watch(dialogOpen, async (offen) => {
  ...
  if (props.withScreenshot) await doCapture()
})
...
// einen Bereichswechsel verschwindet.
watch(zeigeFotoBereich, (zeigt) => { if (!zeigt) kameraStoppen() })

// ── Speichern: erst jetzt entsteht das Ticket ────────────────────────
  ...
  try {
    ...
    for (const a of anhaenge) {
      try {
        ...
        // draft=true: die Anhänge werden nicht einzeln gemeldet – die eine
        // „Neues Ticket"-Meldung (vom Anlegen oben) deckt sie mit ab.
        await api.post(`/api/tickets/${ticket.id}/anhaenge`, fd, { params: { draft: true } })
      } catch (e) {
    ...
    emit('created', ticket)
    // Listen, die sich global aktualisieren (Tickets-Seite, Aufgaben-Badges).
